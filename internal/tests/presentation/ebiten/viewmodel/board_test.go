...
func TestMapperInitialBoardOrientation(t *testing.T) {
	...
	board := mapper.Map(snapshot, false)
...
func TestMapperSelectionAndLegalTargets(t *testing.T) {
	...
	board := mapper.Map(service.Snapshot(), false)
...
func TestMapperBlackPerspectiveOrientation(t *testing.T) {
	mapper := viewmodel.NewMapper(theme.NewTheme())
	snapshot := gameplay.NewService().Snapshot()

	board := mapper.Map(snapshot, true)

	a1 := squareByAlgebraic(t, board, "a1")
	if a1.X != 560 || a1.Y != 0 {
		t.Fatalf("want a1 at (560, 0), got (%d, %d)", a1.X, a1.Y)
	}

	h8 := squareByAlgebraic(t, board, "h8")
	if h8.X != 0 || h8.Y != 560 {
		t.Fatalf("want h8 at (0, 560), got (%d, %d)", h8.X, h8.Y)
	}
}

func TestMapperBlackPerspectiveAxisLabels(t *testing.T) {
	mapper := viewmodel.NewMapper(theme.NewTheme())
	snapshot := gameplay.NewService().Snapshot()

	board := mapper.Map(snapshot, true)

	if got := board.FileLabels[0].Text; got != "h" {
		t.Fatalf("want first file label h, got %s", got)
	}

	if got := board.FileLabels[7].Text; got != "a" {
		t.Fatalf("want last file label a, got %s", got)
	}

	if got := board.RankLabels[0].Text; got != "1" {
		t.Fatalf("want top rank label 1, got %s", got)
	}

	if got := board.RankLabels[7].Text; got != "8" {
		t.Fatalf("want bottom rank label 8, got %s", got)
	}
}
